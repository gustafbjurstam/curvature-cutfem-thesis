...
#define FACE_GHOST // SCRIPT_MARKER2


// #define VOL_GHOST
// #define FACE_GHOST

// #define NORMAL_VOL // This is normally off.

#if defined(VOL_GHOST)
    const bool patching = true;
    const bool facing = false;
#elif defined(FACE_GHOST)
    const bool patching = false;
    const bool facing = true;
#else
    #error "No stabilization type defined"
#endif

#if defined(NORMAL_VOL)
...
namespace elliptic_torus {
    ...
    // Stabilisation
    const double tauP[] = {0.0,1e-4, 1e-3, 1e-2};
    const double tauF[] = {0.0,1e-4, 1e-3, 1e-2};

    const double tauE = 1e-4;
...
public:
    ...
    void elem_stab(const double tau) {
        ...
        auto grad_norm = sqrt(phi_dx * phi_dx + phi_dy * phi_dy + phi_dz * phi_dz); // Added Z
    ...
    double get_error(const fun_t& approx) {
        ...
        return L2normSurf(approx, fun_curvature, interface_, 0, 3);
    }
    ...
    double normal_error_max() {
        ...
        double error3 = max_norm_surface(nz, fun_nz_max, interface_, 0.0); // Added Z-evaluation
...
int main(int argc, char **argv) {
    ...
    const std::string base_output_path = "../output_files/curvature_p1_3d/" + example_name + "/";
    ...
    for (int i = 0; i < n_refinements; ++i) {
        ...
        bool export_matrix = (i < int(1.5*floor(n_refinements / 2.0)));
        ...
        for (int j = 0; j < stab_cases; ++j) {
            {
                ...
                if (patching){
                    curvature.patch_stab(tauP[j]);
                } else if (facing) {
                    curvature.face_stab(tauF[j]);
                }
                if (eleming) {
                    curvature.elem_stab(tauE);
                }
                ...
                if (MPIcf::IamMaster() && export_matrix) {
                    ...
                    // Call without CurvV.NbDoF()
                    curvature.export_global_matrix(base_filename);
                }
    ...
    if (MPIcf::IamMaster()) {
        ...
        if (output_data.is_open()) {
            ...
            std::string filename_base = example;
            if (patching) {
            ...
            // Print the correct stabilisation parameter sweep
            for (int j = 0; j < stab_cases; ++j) {
                double current_tau = patching ? tauP[j] : tauF[j];
